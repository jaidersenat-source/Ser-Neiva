...
<div id="seccion-1" class="scroll-mt-20 bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden mb-5">
    ...
    <div class="p-5 space-y-5">
        ...
        @php $userIglesia = auth()->user()?->iglesia ?? null; @endphp
        <div>
            ...
            @if($userIglesia)
                <div class="flex items-center gap-3">
                    <div class="w-8 h-8 rounded-lg flex items-center justify-center text-white font-bold"
                        ...
                        {{ strtoupper(substr($userIglesia->official_name ?? '?', 0, 1)) }}
                    </div>
                    <div class="min-w-0">
                        <p class="text-sm font-semibold text-slate-800">{{ $userIglesia->official_name }}</p>
                        <p class="text-xs text-slate-400">Iglesia vinculada a tu cuenta</p>
                    </div>
                </div>
                <input type="hidden" name="iglesia_id" value="{{ old('iglesia_id', $evento->iglesia_id ?? $userIglesia->id) }}">
            @else
                <div class="relative">

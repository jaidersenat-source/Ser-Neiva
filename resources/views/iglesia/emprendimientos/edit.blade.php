...
@section('page-title', 'Editar Emprendimiento')
@section('page-subtitle', 'Modificar los datos del emprendimiento')
...
<div class="relative mb-6 rounded-2xl overflow-hidden"
    ...
    <div class="relative z-10 px-6 py-5 sm:px-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-white font-bold text-lg leading-tight">Editar Emprendimiento</h2>
                <p class="text-xs mt-0.5" style="color: rgba(167,243,208,0.9);">
                    Modificar los datos del emprendimiento
                </p>
            </div>
        </div>
        <div class="hidden sm:flex items-center gap-2 text-xs" style="color: rgba(255,255,255,0.55);">
            <a href="{{ route('iglesia.emprendimientos.index') }}" class="hover:text-white transition-colors font-medium">Emprendimientos</a>
...
<div class="max-w-3xl">
    ...
    <form action="{{ route('iglesia.emprendimientos.update', $emprendimiento) }}" method="POST" enctype="multipart/form-data" novalidate>
        @csrf
        @method('PUT')
        @include('iglesia.emprendimientos._form')

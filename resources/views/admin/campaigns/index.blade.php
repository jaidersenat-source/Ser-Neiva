                ...
                @foreach($campaigns as $campaign)
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="px-5 py-3 text-right">
                            ...
                            @if(!$campaign->isSent())
                                <form method="POST" action="{{ route('admin.campaigns.destroy', $campaign) }}" style="display:inline-block;margin-left:8px;" onsubmit="return confirm('¿Eliminar esta campaña? Esta acción no se puede deshacer.');">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="inline-flex items-center gap-1 px-3 py-1.5 text-[11px] font-semibold rounded-lg transition-all hover:opacity-80" style="background:#FFEAEA; color:#DC2626;">

...
<table class="data-table">
    <thead>
        <tr>
            ...
            <th style="width:70px;">Corregimiento</th>
            <th style="width:95px;">Pastor / Sacerdote</th>
            <th style="width:70px;">Fecha Nacimiento Líder</th>
            <th style="width:68px;">Teléfono</th>
            <th style="width:68px;">Email</th>
            <th style="width:68px;">Celular Institucional</th>
            <th style="width:68px;">Correo Institucional</th>
            <th style="width:68px;">Entidad Registrada</th>
            <th style="width:68px;">Promedio Asistentes</th>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td style="font-size:7.5px;">{{ $iglesia->direccion }}</td>
            <td class="c">{{ $iglesia->comuna ?? '—' }}</td>
            <td class="c">{{ $iglesia->corregimiento ?? '—' }}</td>
            <td>{{ $iglesia->pastor_sacerdote ?? '—' }}</td>
            <td class="c">{{ $iglesia->fecha_nacimiento_lider ? \Carbon\Carbon::parse($iglesia->fecha_nacimiento_lider)->format('d/m/Y') : '—' }}</td>
            <td class="c">{{ $iglesia->telefono ?? '—' }}</td>
            <td class="c">{{ $iglesia->email ?? '—' }}</td>
            <td class="c">{{ $iglesia->celular_institucional ?? '—' }}</td>
            <td class="c">{{ $iglesia->correo_institucional ?? '—' }}</td>
            <td class="c">{{ $iglesia->entidad_registrada_colombia ?? '—' }}</td>
            <td class="c">{{ $iglesia->promedio_asistentes ?? '—' }}</td>
            <td style="font-size:7.5px;">{{ $iglesia->descripcion ?? '—' }}</td>
            <td class="c">

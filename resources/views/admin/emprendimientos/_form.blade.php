...
@push('scripts')
    ...
    (function(){
        ...
        // iglesia select fly-to
        const iglesiaSelect = document.getElementById('iglesia_id');
        function volarAIglesia(opt){ if(!opt) return; const lat = parseFloat(opt.dataset.lat), lng = parseFloat(opt.dataset.lng); if(!isNaN(lat)&&!isNaN(lng)&&lat!==0&&lng!==0){ map.flyTo([lat,lng],16,{duration:1.2}); marker.setLatLng([lat,lng]).openPopup(); latInput.value=lat.toFixed(8); lngInput.value=lng.toFixed(8); const addr = opt.dataset.address||''; if(addr && !dirInput.value.trim()){ dirInput.removeEventListener('input', debouncedGeo); dirInput.value=addr; dirInput.addEventListener('input', debouncedGeo); } } }
        if (iglesiaSelect){ iglesiaSelect.addEventListener('change', function(){ volarAIglesia(this.options[this.selectedIndex]); }); if(iglesiaSelect.value){ const opt = iglesiaSelect.options[iglesiaSelect.selectedIndex]; const eventoLat = parseFloat(latInput.value), eventoLng = parseFloat(lngInput.value); if((!eventoLat && !eventoLng) || (eventoLat===0&&eventoLng===0)) volarAIglesia(opt); } }

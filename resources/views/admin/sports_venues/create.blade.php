...
<div class="relative mb-6 rounded-2xl overflow-hidden"
    ...
    <div class="relative z-10 px-6 py-5 sm:px-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-white font-bold text-lg leading-tight">Nuevo Escenario Deportivo</h2>
                <p class="text-xs mt-0.5" style="color: rgba(253,186,116,0.85);">
                    Registrar nuevo espacio deportivo en el sistema
                </p>
            </div>
        </div>
        {{-- Breadcrumb --}}
...
<div class="max-w-2xl">
    ...
    @if($errors->any())
        <div class="mb-5 bg-white rounded-2xl border border-red-200 shadow-sm overflow-hidden">
            <div class="flex items-center gap-3 px-5 py-3 border-b border-red-100"
    ...
    <form action="{{ route('admin.sports_venues.store') }}" method="POST" enctype="multipart/form-data" novalidate>
        @csrf
